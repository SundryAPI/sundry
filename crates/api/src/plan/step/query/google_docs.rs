...
const GATED_TABLE_MAPPINGS: [(&str, &str); 0] = [
...
pub struct GoogleDocsText2SQLSuccessResponse {
    pub sql: String,
...
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct GoogleDocsText2SQLFailureResponse {
    pub error: String,
}

#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(untagged)]
pub enum GoogleDocsText2SQLResponse {
    Success(GoogleDocsText2SQLSuccessResponse),
    Failure(GoogleDocsText2SQLFailureResponse),
}

#[derive(Debug, Clone, Serialize)]
pub struct GoogleDocsQueryInput<'a, 'b> {
    query: &'a str,
    user: &'b str,
}
...
pub async fn execute(
    ...
    let prompt = GoogleDocsQueryInput {
        query,
        user: &github_username_for_user_id(auth_data.user_key.user_id, pool)
            .await?
            .ok_or(PlanError::RequestingAccessToNonSyncedSource(Source::Google))?,
    };

    let text_2_sql: GoogleDocsText2SQLResponse = MODELS
        .text_2_sql
        .chat_with_value(
    ...
    match text_2_sql {
        GoogleDocsText2SQLResponse::Success(text2_sqlsuccess_response) => {
            let sql_query =
        ...
        GoogleDocsText2SQLResponse::Failure(git_hub_text2_sqlfailure_response) => {
